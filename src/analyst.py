...
<div class="modal-overlay" id="methodology-modal">
  ...
  <div class="modal-content" style="max-width:680px;">
    ...
    <div class="gloss-item">
      ...
      <div class="gloss-def">
        A pairwise win probability matrix is built for all 68 teams. The primary engine is a <b>calibrated Logistic Regression</b> (with isotonic calibration) trained on <b>738 NCAA tournament games (2011&ndash;2025, excluding 2012/2020)</b>. 14 candidate features are extracted from the team stats above; L1 (Lasso) screening selects <b>8 active features</b>: seed difference, AdjO differential, tempo differential, seed &times; AdjEM interaction, top-25 win% gap, underdog top-25 win%, Barthag gap, and WAB gap. When the trained model is unavailable, we fall back to historical seed-vs-seed upset rates.
      </div>
    </div>
    ...
    <div class="gloss-item">
      ...
      <div class="gloss-def">
        We identify up to <b>24 champion candidates</b> (seeds 1&ndash;6) ranked by pool-adjusted value (title probability divided by expected number of opponents picking the same champion). For each candidate, we generate scenarios at multiple chaos levels:
        ...
        Each (champion, chaos level) cell is expanded across multiple Final Four compositions, cinderella variants, and chaos-region permutations &mdash; roughly <b>2,000 scenarios</b> in total. Deduplication removes identical brackets before evaluation.
      </div>
    </div>
    ...
    <div class="gloss-item">
      ...
      <div class="gloss-def">
        Each candidate bracket is evaluated via <b>Monte Carlo simulation</b> (configurable via <code>--sims</code>; 50,000+ for tight pools, 5,000+ for large fields). In each simulation:
        <ol style="margin:6px 0 0 16px; line-height:1.8;">
...
function updateModelPrediction() {
  ...
  resultEl.innerHTML =
    ...
    '<div style="font-size:10px;color:#556677;text-align:center;margin-top:6px;">' +
      (calibrated ? '\u2020 Calibrated logistic regression' : '\u2020 Logistic regression (uncalibrated)') +
    '</div>';
